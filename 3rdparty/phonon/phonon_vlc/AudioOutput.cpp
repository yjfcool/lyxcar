...
void AudioOutput::setVolume( qreal volume ) 
{
    if( p_vlc_instance )
    {
		libvlc_audio_set_volume( p_vlc_instance, (int) ( f_volume * 100 ), p_vlc_exception );
		checkException();
		f_volume = volume;
...
bool AudioOutput::setOutputDevice( int device )
{
    ...
    if( device >= 0 && device < deviceList.size() )
    {
        ...
        libvlc_audio_output_set(p_vlc_instance, (char *) deviceList.at( device ).nameId.data());
        printf("set aout %s\n", deviceList.at( device ).nameId.data() );
/*
